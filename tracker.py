...
def edit_expense():
    ...
    with open("expenses.txt","r") as f:
        ...
        for i, line in enumerate(lines):
            print(f"{i+1} - {line}" )
            total = i+1
    choice = int(input("Which line do you want to edit "))
    try:
        if choice <= total:
             with open("expenses.txt", "w") as f:
                 for i, line in enumerate(lines):
                        if i != choice - 1:
                            ...
                            try:
                                ...
                                amount = float(input("Please enter the new value in numbers ₹ "))
                            category = input("Category(food, transport, bills): ").lower()
                            description = input("What was it for? ")
                            f.write(f"{line[0:10]} | {category} | {amount:.2f} | {description}\n")
        else:
            print(f"Enter a entry that exist")
    except ValueError:
        print("Please enter a valid number")
...
while True:
    user_choice = input( "What do you want to do add[a] or view summary[v] or view current month summary[c] or delete_expense[d] or search_expense(s) or edit_expense(e) or quit[q]?\n").lower()
    if user_choice == "a":
        add_expense()
    elif user_choice == "v":
        view_summary()
